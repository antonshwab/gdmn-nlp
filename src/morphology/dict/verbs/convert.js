...
const verbs = rawFile.replace(/́/g, '').split('{{ШаблонДемо').reduce( (p, s, idx) => {
  if (s) {
    const rg = /\s*\|имя=гл ru (.+)\s(?:\|основа[ ]?=[ ]?(.+)\s){1}(?:\|основа1[ ]?=[ ]?(.+)\s)?(?:\|основа2[ ]?=[ ]?(.+)\s)?(?:\|основа3[ ]?=[ ]?(.+)\s)?(?:\|основа4[ ]?=[ ]?(.+)\s)?(?:\|основа5[ ]?=[ ]?(.+)\s)?\|слова[ ]?=[ ]?(?:\[\[([^\]]+){1}\]\](?:[ ]*\([^)]+\))?[, ]{0,2}){1}(?:\[\[([^\]]+){1}\]\](?:[ ]*\([^)]+\))?[, ]{0,2})?(?:\[\[([^\]]+){1}\]\](?:[ ]*\([^)]+\))?[, ]{0,2})?(?:\[\[([^\]]+){1}\]\](?:[ ]*\([^)]+\))?[, ]{0,2})?(?:\[\[([^\]]+){1}\]\](?:[ ]*\([^)]+\))?[, ]{0,2})?\s\}\}/g;
...
fs.writeFileSync('./rusconjend.txt', verbs.reduce( (p, l) => {
    ...
    const verbSuffixes = ['нуть', 'сть', 'дти', 'ать', 'ить', 'уть', 'еть', 'ыть', 'ять', 'оть', 'ть', 'чь', 'ти'];
    const c = l[0].replace('\'', '\\\'');
